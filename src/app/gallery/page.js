...
// Fallback data for when API is unavailable or user not authenticated
...
export default function PhotoGallery() {
  ...
  const fetchGalleryItems = async () => {
    try {
      ...
      // Use fallback data if API fails (user not logged in or server down)
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {galleryItems.map((item, index) => (
              <div
                ...
                className="relative cursor-pointer group overflow-hidden rounded-xl shadow-md hover:shadow-xl transition-shadow duration-300"
              >
